...
export const ProfilePage = () => {
  ...
  const dynamicLinks = Array.isArray(profile?.social_links)
    ? profile.social_links.filter((link: any) => link?.url)
    : [];

  const fallbackLinks = [
    profile?.website_url ? { platform: 'portfolio', url: profile.website_url } : null,
    ...
    profile?.upwork_url ? { platform: 'upwork', url: profile.upwork_url } : null,
    profile?.fiverr_url ? { platform: 'fiverr', url: profile.fiverr_url } : null
  ].filter(Boolean) as Array<{ platform: string; url: string }>;

  const visibleLinks = dynamicLinks.length > 0 ? dynamicLinks : fallbackLinks;
        ...
        <CardContent className="p-6">
          <div className="flex flex-col md:flex-row md:items-end gap-6 -mt-16">
            ...
            <div className="flex-1">
              ...
              {profile.headline && <p className="text-ink-4">{profile.headline}</p>}
